...
(function(root, factory) {
    ...
    $.extend(EmojiPicker.prototype, {
        ...
        updatePosition: function() {
            ...
            */
            ...
            if (positionedParent.width() < elOffset.left + this.$picker.width() + 10)
                elOffset.left =
                    Math.max(positionedParent.width() - this.$picker.width() - 17, 10); //-17 for scrollbar width
        ...
        emojiMouseover: function(e) {
            ...
            $shortcode.find('.info').show().html('<div style="display: table-cell; vertical-align: middle;">' +
                this.emoji.replace_colons(':' + emojiShortcode + ':') +
                '<em style="vertical-align: center;">' +
                emojiShortcode +
                '</em></div>');
        },
        ...
        emojiTabMouseover: function(e) {
            ...
            var categoryHtml = '<em class="tabTitle">' +
                categoryTitle +
                ' <span class="count">(' +
                categoryCount +
                ' emojis)</span></em>';
        ...
        emojiScroll: function(e) {
            ...
            $.each(sections, function(key, value) {
                ...
                if (offsetFromTop <= 0) {
                    ...
                    $(section).parents('.emojiPicker').find('nav .tab[data-tab=' + section.className + ']')
                        .addClass('active');
                }
            });
        },
                ...
                $.each($.fn.emojiPicker.emojis, $.proxy(function(i, emoji) {
                    for (var j = 0; j < emoji.shortcodes.length; j++) {
                        ...
                        if (shortCode.indexOf(searchTerm) !== -1) {
                            results.push('<em class="emoji ' +
                                emoji.name +
                                '">' +
                                this.emoji.replace_colons(':' + emoji.name + ':') +
                                '</em>');
                            return;
                        }
                    }
    ...
    function getPickerHTML(emojiConverter) {
        ...
        if (localStorageSupport) {
            ...
            nodes.push('<section class="recent" data-count="' +
            ...
            for (var i = recentlyUsedEmojis.length - 1; i > -1; i--) {
                nodes.push('<em class="emoji ' +
                    recentlyUsedEmojis[i] +
                    '">' +
                    emojiConverter.replace_colons(':' + recentlyUsedEmojis[i] + ':') +
                    '</em>');
            }
        ...
        for (var i = 0; i < categories_length; i++) {
            ...
            for (var j = 0; j < category_length; j++) {
                ...
                nodes.push('<em class="emoji ' +
                    emoji.name +
                    '">' +
                    emojiConverter.replace_colons(':' + emoji.name + ':') +
                    '</em>');
            }
            nodes.push('</div></section>');
        }
    ...
    function generateEmojiOfDay(emojiConverter) {
        ...
        var emoji = emojis[i];
        return 'Daily Emoji: <span class="eod">' +
            emojiConverter.replace_colons(':' + emoji.name + ':') +
            ' <span class="emojiName">' +
            emoji.name +
            '</span></span>';
    }
